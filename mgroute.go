// Required Notice: Copyright (c) 2026 Murilo Gomes Julio. All Rights Reserved. (https://profmugomes.com.br)
...
func New(requestURI string) *sRoute {
	path := "/"
...
func (r *sRoute) GetArrayURLs() []string {
	return r.urlParts
}

// Retorna a URL completa
func (r *sRoute) GetFullURL() string {
	return strings.TrimLeft(r.fullURL, "/")
}

// Verifica se a URL existe
func (r *sRoute) CheckURL(pattern string) bool {
	re := regexp.MustCompile("^" + pattern + "$")
	return re.MatchString(r.fullURL)
}

// Retorna a parte da URL pelo índice
func (r *sRoute) GetURL(index int) string {
...
func (r *sRoute) GetFirstURL() string {
	return r.GetURL(0)
}

// Retorna a penúltima parte da URL
func (r *sRoute) GetPenultimateURL() string {
	if len(r.urlParts) < 2 {
		return ""
	}
...
func (r *sRoute) GetLastURL() string {
	if len(r.urlParts) == 0 {
		return ""
	}
...
func (r *sRoute) GetPart(pattern string, function func(args ...string)) {
	sURL := "/"
	if r.fullURL != "" {
		sURL = r.fullURL
	}
...
func (r *sRoute) GetError(handler func()) {
	if r.error404 {
		handler()
	}
}
